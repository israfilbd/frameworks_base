...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    @VisibleForTesting
    int mRepeatingVolumeKeyCode = KeyEvent.KEYCODE_UNKNOWN;
    ...
    private static final int MSG_VOLUME_KEY_REPEAT = 102;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_CAMERA_LONG_PRESS:
                    mIsLongPress = true;
                    break;
                case MSG_VOLUME_KEY_REPEAT: {
                    int repeatKeyCode = msg.arg1;
                    if (mRepeatingVolumeKeyCode == repeatKeyCode) {
                        dispatchVolumeRepeat(repeatKeyCode);
                        Message nextMsg = mHandler.obtainMessage(MSG_VOLUME_KEY_REPEAT, repeatKeyCode, 0);
                        nextMsg.setAsynchronous(true);
                        mHandler.sendMessageDelayed(nextMsg, ViewConfiguration.getKeyRepeatDelay());
                    }
                    break;
                }
            }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        if (down && mRepeatingVolumeKeyCode != KeyEvent.KEYCODE_UNKNOWN && keyCode != mRepeatingVolumeKeyCode) {
            mRepeatingVolumeKeyCode = KeyEvent.KEYCODE_UNKNOWN;
            mHandler.removeMessages(MSG_VOLUME_KEY_REPEAT);
        }
                    ...
                    if (keyCode == KeyEvent.KEYCODE_VOLUME_UP || keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
                        if (down) {
                            if (event.getRepeatCount() == 0) {
                                boolean musicControlsEnabled = LineageSettings.System.getIntForUser(
                                        mContext.getContentResolver(), LineageSettings.System.VOLBTN_MUSIC_CONTROLS,
                                        0, UserHandle.USER_CURRENT) == 1;
                                if (!musicControlsEnabled) {
                                    mRepeatingVolumeKeyCode = keyCode;
                                    mHandler.removeMessages(MSG_VOLUME_KEY_REPEAT);
                                    Message msg = mHandler.obtainMessage(MSG_VOLUME_KEY_REPEAT, keyCode, 0);
                                    msg.setAsynchronous(true);
                                    mHandler.sendMessageDelayed(msg, ViewConfiguration.getKeyRepeatTimeout());
                                }
                            }
                        } else {
                            if (keyCode == mRepeatingVolumeKeyCode) {
                                mRepeatingVolumeKeyCode = KeyEvent.KEYCODE_UNKNOWN;
                                mHandler.removeMessages(MSG_VOLUME_KEY_REPEAT);
                            }
                        }
    ...
    private void dispatchVolumeRepeat(int keyCode) {
        long now = SystemClock.uptimeMillis();
    ...
    public void startedGoingToSleep(int displayGroupId,
        ...
        if (mRepeatingVolumeKeyCode != KeyEvent.KEYCODE_UNKNOWN) {
            mRepeatingVolumeKeyCode = KeyEvent.KEYCODE_UNKNOWN;
            mHandler.removeMessages(MSG_VOLUME_KEY_REPEAT);
        }
