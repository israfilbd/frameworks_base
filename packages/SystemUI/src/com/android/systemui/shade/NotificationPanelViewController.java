...
public final class NotificationPanelViewController implements
    ...
    private static final int QS_HEAVY_NOTIFICATION_COUNT = 5;
    private static final long QS_RELAYOUT_BOOST_MIN_INTERVAL_HEAVY_MS = 250L;
    private static final long QS_RELAYOUT_BOOST_MIN_INTERVAL_LIGHT_MS = 500L;
    private static final long QS_INTERACTION_BOOST_MIN_INTERVAL_HEAVY_MS = 350L;
    private static final long QS_INTERACTION_BOOST_MIN_INTERVAL_LIGHT_MS = 700L;
    private static final long QS_INTERACTION_BOOST_MIN_INTERVAL_NO_CONTENT_MS = 1200L;
    private static final int QS_INTERACTION_BOOST_MIN_DURATION_MS = 120;
    private static final int QS_INTERACTION_BOOST_MAX_DURATION_NO_CONTENT_MS = 240;
    private static final int QS_INTERACTION_BOOST_MAX_DURATION_LIGHT_MS = 420;
    private static final int QS_INTERACTION_BOOST_MAX_DURATION_MS = 700;
    ...
    private long mLastQsRelayoutBoostUptime = -QS_RELAYOUT_BOOST_MIN_INTERVAL_LIGHT_MS;
    private long mLastInteractionBoostUptime = -QS_INTERACTION_BOOST_MIN_INTERVAL_LIGHT_MS;
    ...
    private boolean isHeavyQsContent() {
        return mMediaDataManager.hasActiveMedia()
                || mActiveNotificationsInteractor.getAllNotificationsCountValue()
                >= QS_HEAVY_NOTIFICATION_COUNT;
    }

    private long getRelayoutBoostMinIntervalMs() {
        return isHeavyQsContent()
                ? QS_RELAYOUT_BOOST_MIN_INTERVAL_HEAVY_MS
                : QS_RELAYOUT_BOOST_MIN_INTERVAL_LIGHT_MS;
    }

    private long getInteractionBoostMinIntervalMs() {
        return isHeavyQsContent()
                ? QS_INTERACTION_BOOST_MIN_INTERVAL_HEAVY_MS
                : QS_INTERACTION_BOOST_MIN_INTERVAL_LIGHT_MS;
    }

    private boolean shouldBoostFramesForRelayout(boolean forceForShadeTransition) {
        if (mDozing || !mView.isAttachedToWindow() || !hasVisibleNotifications()) {
            return false;
        }
        if (!forceForShadeTransition) {
            boolean isQsInteractionActive = mQsController.getExpanded()
                    || mQsController.isExpansionAnimating()
                    || isTracking()
                    || mIsFlinging
                    || mInstantExpanding;
            if (!isQsInteractionActive) {
                return false;
            }
        }
        long now = mSystemClock.uptimeMillis();
        if (now - mLastQsRelayoutBoostUptime < getRelayoutBoostMinIntervalMs()) {
            return false;
        }
        mLastQsRelayoutBoostUptime = now;
        return true;
    }

    private void boostFramesDuringRelayout(String reason, boolean forceForShadeTransition) {
        if (shouldBoostFramesForRelayout(forceForShadeTransition)) {
            DejankUtils.notifyRendererOfExpensiveFrame(mView, reason + ":preLayout");
            mView.requestLayout();
            DejankUtils.notifyRendererOfExpensiveFrame(mView, reason + ":postLayout");
            return;
        }
        mView.requestLayout();
    }

    private boolean isHighValueBoostReason(String reason) {
        return "notifyExpandingStarted".equals(reason)
                || "flingToHeight".equals(reason)
                || "springBack".equals(reason)
                || "maybeVibrateOnOpening".equals(reason);
    }

    private boolean shouldBoostInteraction(String reason) {
        if (mDozing || !mPowerManager.isInteractive() || !mView.isAttachedToWindow()) {
            return false;
        }
        boolean isQsInteractionActive = mExpanding
                || mIsFlinging
                || isTracking()
        ...
        boolean hasVisibleContent = hasVisibleNotifications();
        if (!hasVisibleContent && !isHighValueBoostReason(reason)) {
            return false;
        }
        long minInterval = hasVisibleContent
                ? getInteractionBoostMinIntervalMs()
                : QS_INTERACTION_BOOST_MIN_INTERVAL_NO_CONTENT_MS;
        return mSystemClock.uptimeMillis() - mLastInteractionBoostUptime
                >= minInterval;
    }

    private void boostInteraction(int durationMs, String reason) {
        if (!shouldBoostInteraction(reason)) {
            return;
        }
        ...
        boolean hasVisibleContent = hasVisibleNotifications();
        if (!isHeavyQsContent()
                && (reason.endsWith(":onAnimationEnd") || reason.endsWith(":onAnimationCancel"))) {
            return;
        }
        int boundedDuration = MathUtils.constrain(durationMs,
                QS_INTERACTION_BOOST_MIN_DURATION_MS,
                !hasVisibleContent
                        ? QS_INTERACTION_BOOST_MAX_DURATION_NO_CONTENT_MS
                        : (isHeavyQsContent()
                                ? QS_INTERACTION_BOOST_MAX_DURATION_MS
                                : QS_INTERACTION_BOOST_MAX_DURATION_LIGHT_MS));
        pmi.setPowerBoost(Boost.INTERACTION, boundedDuration);
        mLastInteractionBoostUptime = mSystemClock.uptimeMillis();
        debugLog("boostInteraction reason=%s durationMs=%d", reason, boundedDuration);
    }

    @Override
    public void transitionToExpandedShade(long delay) {
        mNotificationStackScrollLayoutController.goToFullShade(delay);
        boostFramesDuringRelayout("transitionToExpandedShade", true);
    ...
    void flingToHeight(float vel, boolean expand, float target,
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationCancel(Animator animation) {
                mCancelled = true;
                boostInteraction(220, "flingToHeight:onAnimationCancel");
            }

            @Override
            public void onAnimationEnd(Animator animation) {
                boostInteraction(220, "flingToHeight:onAnimationEnd");
        ...
        animator.start();
        boostInteraction((int) animator.getDuration(), "flingToHeight");
    }
    ...
    public void expand(boolean animate) {
        if (isFullyCollapsed() || isCollapsing()) {
            ...
            boostFramesDuringRelayout("expand", true);
        }
    ...
    void notifyExpandingStarted() {
        if (!mExpanding) {
            ...
            boostInteraction(700, "notifyExpandingStarted");
        }
    }
    ...
    private void maybeVibrateOnOpening(boolean openingWithTouch) {
        if (mVibrateOnOpening && mBarState != KEYGUARD && mBarState != SHADE_LOCKED) {
            if (!openingWithTouch || !mHasVibratedOnOpen) {
                boostInteraction(500, "maybeVibrateOnOpening");
                performHapticFeedback(HapticFeedbackConstants.GESTURE_START);
    ...
    private void springBack() {
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            @Override
            public void onAnimationEnd(Animator animation) {
                boostInteraction(220, "springBack:onAnimationEnd");
        ...
        setAnimator(animator);
        animator.start();
        boostInteraction((int) animator.getDuration(), "springBack");
    }
    ...
    public void updateTouchableRegion() {
        ...
        boostFramesDuringRelayout("updateTouchableRegion", false);
