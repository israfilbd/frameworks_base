...
public class QuickSettingsControllerImpl implements QuickSettingsController, Dumpable {
    ...
    private void onExpansionStarted() {
        ...
        boostInteraction(320, "onExpansionStarted");
    ...
    private boolean hasVisibleQsContent() {
    ...
    private long getInteractionBoostMinIntervalMs(boolean hasVisibleContent) {
        if (!hasVisibleContent) {
            return QS_INTERACTION_BOOST_MIN_INTERVAL_NO_CONTENT_MS;
        }
    ...
    private void boostInteraction(int durationMs, String reason) {
        if (mDozing || !mPanelView.isAttachedToWindow()) {
            return;
        }
        if (!isTracking() && !mAnimating && !isExpansionAnimating()) {
            return;
        }
        boolean hasVisibleContent = hasVisibleQsContent();
        long now = SystemClock.uptimeMillis();
        if (now - mLastInteractionBoostUptime < getInteractionBoostMinIntervalMs(hasVisibleContent)) {
            return;
        }
        ...
        mLastInteractionBoostUptime = now;
        mShadeLog.d("QS boostInteraction reason=" + reason + " durationMs=" + boundedDuration);
    }
    ...
    public float calculateNotificationsTopPadding(boolean isShadeExpanding,
        ...
        boolean keyguardShowing = mBarState == KEYGUARD;
        if (mQsSplitShadeEnabledLegacy && mBarState == StatusBarState.SHADE) {
            return mQuickQsHeaderHeight + mResources.getDimensionPixelSize(R.dimen.notification_side_paddings) / 2;
        }
        if (mSplitShadeEnabled) {
