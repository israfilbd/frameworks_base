...
constructor(
    ...
    fun updateQsHeaderClockDateVisibility() {
        val customEnabled = qsClockStyle != 0
        val textColor = if (customEnabled) AndroidColor.TRANSPARENT else context.getColor(R.color.island_title_color)

        val colorStateList = ColorStateList.valueOf(textColor)
        clock.setTextColor(colorStateList)
        date.setTextColor(colorStateList)

        // Hide the default clock/date when a custom header clock is enabled to avoid overlap
        clock.isVisible = !customEnabled
        date.isVisible = !customEnabled
    }
