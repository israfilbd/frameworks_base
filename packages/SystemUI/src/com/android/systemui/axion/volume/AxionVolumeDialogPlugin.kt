...
class AxionVolumeDialogPlugin @Inject constructor(
    ...
    companion object {
        private const val DEFAULT_DISMISS_TIMEOUT_MS = 3000L
    }
    ...
    fun rescheduleAutoDismiss() {
        autoDismissJob?.cancel()
        val timeout = getDismissTimeoutMs()
    ...
    private fun getDismissTimeoutMs(): Long {
        return Settings.Secure.getIntForUser(
            context.contentResolver,
            Settings.Secure.VOLUME_DIALOG_DISMISS_TIMEOUT,
            DEFAULT_DISMISS_TIMEOUT_MS.toInt(),
            UserHandle.USER_CURRENT,
        ).coerceAtLeast(0).toLong()
    }

    private fun performVolumeKeyHaptic() {
        if (!isVolumeDialogHapticsEnabled()) return
    ...
    private fun isVolumeDialogHapticsEnabled(): Boolean {
        return Settings.Secure.getIntForUser(
            context.contentResolver,
