...
fun RingerSliderWidget(
    ...
    Box(
        modifier = modifier
            ...
            .then(
                if (isDozing)
                    Modifier.border(theme.dozeStroke, Color.White, RoundedCornerShape(50.dp))
                else border
            )
.pointerInput(Unit) {
    detectTapGestures(
        ...
        BoxWithConstraints(modifier = Modifier.fillMaxWidth()) {
            ...
            Box(
                modifier = Modifier
                    ...
                    .then(
                        when {
                            isDozing -> Modifier.background(Color.Transparent, CircleShape)
                            isDndEnabled -> if (dndBrush != null)
                                Modifier.background(dndBrush, CircleShape)
                            else
                                Modifier.background(theme.dndBg, CircleShape)
                            else -> if (activeBrush != null)
                                Modifier.background(activeBrush, CircleShape)
                            else
                                Modifier.background(theme.activeBg, CircleShape)
                        }
                    )
                    .then(
                        when {
                            ...
                            isDndEnabled ->
                                Modifier.border(2.dp, theme.dndBg, CircleShape)
                            else ->
