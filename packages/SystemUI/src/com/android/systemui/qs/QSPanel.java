...
public class QSPanel extends LinearLayout implements Tunable {
    ...
    public static final String OOS_QS_STYLE =
            "system:" + Settings.System.OOS_QS_STYLE;
    ...
    protected void updatePadding() {
        ...
        int paddingTop = res.getDimensionPixelSize(R.dimen.qs_panel_padding_top)
                        + (QSUtils.shouldShowOosQsLayout(mContext)
                            ? res.getDimensionPixelSize(R.dimen.oos_qs_panel_padding_top)
                            : 0);
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        updateOosQsLayoutVisibility();
    }

    private void updateOosQsLayoutVisibility() {
        if (mOpQsLayoutShade != null) {
            boolean isLandscape = mContext.getResources().getConfiguration().orientation 
                == Configuration.ORIENTATION_LANDSCAPE;
            if (mOpQsLayoutShade != null) {
                mOpQsLayoutShade.setVisibility(
                    QSUtils.isOosQsLayoutEnabled(mContext) && isLandscape
                    ? View.VISIBLE
                    : View.GONE);
            }
        }
    ...
    protected void onFinishInflate() {
        ...
        mOpQsLayoutShade = findViewById(R.id.qs_op_header_layout_shade);
    }
    ...
    private void switchAllContentToParent(ViewGroup parent, QSTileLayout newLayout) {
        ...
        if (mOpQsLayoutShade != null
            && QSUtils.isOosQsLayoutEnabled(mContext) && isLandscape) {
            switchToParent(mOpQsLayoutShade, parent, index);
            index++;
        }
    ...
    private void reAttachMediaHost(ViewGroup hostView, boolean horizontal) {
        ...
        if (QSUtils.isOosQsLayoutEnabled(mContext)) {
            if (currentParent != newParent && currentParent != null) {
