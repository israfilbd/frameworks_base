...
public class NotificationStackScrollLayout
    ...
    private static final int HEAVY_SHADE_VISIBLE_CHILD_COUNT = 5;
    private static final long INTERACTION_BOOST_MIN_INTERVAL_HEAVY_MS = 350L;
    private static final long INTERACTION_BOOST_MIN_INTERVAL_LIGHT_MS = 700L;
    private static final long INTERACTION_BOOST_MIN_INTERVAL_NO_CONTENT_MS = 1200L;
    private static final int INTERACTION_BOOST_MIN_DURATION_MS = 120;
    private static final int INTERACTION_BOOST_MAX_DURATION_NO_CONTENT_MS = 240;
    private static final int INTERACTION_BOOST_MAX_DURATION_LIGHT_MS = 420;
    private static final int INTERACTION_BOOST_MAX_DURATION_MS = 700;
    ...
    private long mLastInteractionBoostUptime = -INTERACTION_BOOST_MIN_INTERVAL_LIGHT_MS;
    ...
    protected void fling(int velocityY) {
        ...
        if (getChildCount() > 0) {
            ...
            animateScroll();
            int flingBoostDuration = MathUtils.constrain(Math.abs(velocityY) / 10, 180, 700);
            boostInteraction(flingBoostDuration, "fling");
        }
    }

    private boolean shouldBoostInteraction(String reason) {
        if (!isAttachedToWindow() || !mAnimationsEnabled) {
            return false;
        }
        if (!mPanelTracking && (mScroller == null || mScroller.isFinished())) {
            return false;
        }
        int visibleShadeChildCount = getVisibleShadeChildCount();
        if (visibleShadeChildCount == 0 && !"fling".equals(reason)
                && !"onPanelTrackingStarted".equals(reason)) {
            return false;
        }
        long minInterval = visibleShadeChildCount == 0
                ? INTERACTION_BOOST_MIN_INTERVAL_NO_CONTENT_MS
                : getInteractionBoostMinIntervalMs(visibleShadeChildCount);
        return SystemClock.uptimeMillis() - mLastInteractionBoostUptime
                >= minInterval;
    }

    private long getInteractionBoostMinIntervalMs(int visibleShadeChildCount) {
        return visibleShadeChildCount >= HEAVY_SHADE_VISIBLE_CHILD_COUNT
                ? INTERACTION_BOOST_MIN_INTERVAL_HEAVY_MS
                : INTERACTION_BOOST_MIN_INTERVAL_LIGHT_MS;
    }

    private int getVisibleShadeChildCount() {
        int visibleCount = 0;
        int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            ExpandableView child = getChildAtIndex(i);
    ...
    private void boostInteraction(int durationMs, String reason) {
        ...
        int boundedDuration = MathUtils.constrain(durationMs,
                INTERACTION_BOOST_MIN_DURATION_MS,
                visibleShadeChildCount == 0
                        ? INTERACTION_BOOST_MAX_DURATION_NO_CONTENT_MS
                        : (visibleShadeChildCount >= HEAVY_SHADE_VISIBLE_CHILD_COUNT
                                ? INTERACTION_BOOST_MAX_DURATION_MS
                                : INTERACTION_BOOST_MAX_DURATION_LIGHT_MS));
        pmi.setPowerBoost(Boost.INTERACTION, boundedDuration);
        mLastInteractionBoostUptime = SystemClock.uptimeMillis();
        if (SPEW) {
            Log.v(TAG, "boostInteraction reason=" + reason + " durationMs=" + boundedDuration);
        }
    }
    ...
    void onPanelTrackingStarted() {
        ...
        resetExposedMenuView(true /* animate */, true /* force */);
        boostInteraction(700, "onPanelTrackingStarted");
    }
