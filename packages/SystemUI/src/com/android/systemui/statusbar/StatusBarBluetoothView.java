...
public class StatusBarBluetoothView extends FrameLayout implements StatusIconDisplayable {
    ...
    // Neon flashy colors for dark backgrounds
    ...
    // Muted colors for light backgrounds
    ...
    private void updateBatteryColor() {
        int color;
	if (mBatteryLevel > 50) {
		color = isDarkMode ? mHighBatteryColorLightMuted : mHighBatteryColorDarkNeon;
	} else if (mBatteryLevel > 25) {
		color = isDarkMode ? mLowBatteryColorLightMuted : mLowBatteryColorDarkNeon;
	} else {
		color = isDarkMode ? mCriticalBatteryColorLightMuted : mCriticalBatteryColorDarkNeon;
	}
