...
public class LockScreenWidgetsController implements OmniJawsClient.OmniJawsObserver, MediaSessionManagerHelper.MediaMetadataListener {
    ...
    private final Handler mBgHandler = new Handler(Dependency.get(Dependency.BG_LOOPER));
    ...
    private void toggleBluetoothState() {
        final boolean enable = !mBluetoothController.isBluetoothEnabled();
        updateBtState(enable, null);
        mBgHandler.post(() -> mBluetoothController.setBluetoothEnabled(enable));
    }
    ...
    private void updateBtState() {
        ...
        mBgHandler.post(() -> {
            final String deviceName = mBluetoothController.getConnectedDeviceName();
            mHandler.post(() -> updateBtState(enabled, deviceName));
        });
    }

    private void updateBtState(boolean enabled, @Nullable String deviceName) {
        final String inactiveString = mContext.getResources().getString(BT_LABEL_INACTIVE);
        final boolean isConnected = !TextUtils.isEmpty(deviceName);
