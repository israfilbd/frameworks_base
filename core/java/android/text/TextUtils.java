...
public class TextUtils {
	...
	if (cs instanceof Spanned) {
		...
		for (Object span : spans) {
		    Object resolvedSpan = span;

		    if (resolvedSpan instanceof CharacterStyle) {
		        resolvedSpan = ((CharacterStyle) resolvedSpan).getUnderlying();
		    }

		    if (resolvedSpan instanceof ParcelableSpan) {
		        ParcelableSpan parcelableSpan = (ParcelableSpan) resolvedSpan;
		        int spanTypeId = parcelableSpan.getSpanTypeIdInternal();
		        
		        if (spanTypeId < FIRST_SPAN || spanTypeId > LAST_SPAN) {
		            Log.e(TAG, "External class \"" + parcelableSpan.getClass().getSimpleName()
		                    + "\" is attempting to use the frameworks-only ParcelableSpan interface");
		        } else {
		            p.writeInt(spanTypeId);
		            parcelableSpan.writeToParcelInternal(p, parcelableFlags);
		            writeWhere(p, sp, span);
		        }
		    }
		}
		p.writeInt(0);
