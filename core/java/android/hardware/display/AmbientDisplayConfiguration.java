...
public class AmbientDisplayConfiguration {
    ...
    // If native double tap sensor is available, use the AOSP setting
    if (doubleTapSensorAvailable()) {
        return boolSetting(Settings.Secure.DOZE_DOUBLE_TAP_GESTURE, user,
                mDoubleTapGestureEnabledByDefault ? 1 : 0);
    }
    // If no native sensor, use custom system setting for fallback double tap
    return pulseOnCustomDozeEventEnabled(user); 
    }
